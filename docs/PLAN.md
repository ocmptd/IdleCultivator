...
### Phase 1:核心修炼循环
- [x] `!创建角色`:性别选择、初始属性(男:体魄+5% / 女:神识+5%)、称号
- [x] `!修炼 [功法] [时长]`:双轨制修炼
  - 修炼收益随时长递增(最长 2h,时长越长每小时效率越高),灵石 = 修为÷2
  - 长时修炼(30 分钟~2h)到期需收获,超时溢出按分级惩罚转化
- [x] 修炼自动结算:定时检查任务表,完成时机器人主动推送结果
- [x] 溢出惩罚分级:≤1h→灵尘(×0.3);1-3h→残破法宝(×0.5);>3h→灵石(×0.7)+20% 走火入魔
- [x] `!突破`:修为足够触发,可消耗丹药提升成功率
- [x] `!状态`:境界、属性、进行中修炼、背包、文字形象描述

### Phase 2:养成与个性化
- [x] 修炼方向系统:`!选择方向 剑修/法修/医修`(筑基期后),方向专属加成与技能
- [x] 性别×方向联动功法(焚天诀 / 九霄云衣诀 / 丹心诀 / 冰魄剑诀)
- [x] 道具系统:功法、法宝、丹药、服饰、配饰、材料、加速丹、溢出保护符等
- [x] 形象定制:`!换发型 / !换服饰 / !换配饰`,境界解锁进阶外观
- [x] 形象展示:状态文字描述,后续可选图片生成(`!展示形象`)

### Phase 3:社交与群联动 ✅
- [x] 群活跃度分级(灵气稀薄/平和/充沛/鼎盛 → 效率 ×0.8/1.0/1.2/1.5),`!群状态`
- [x] `!互助 @某人`:双方效率+20%/1h,最多叠 3 次
- [x] 每日灵气潮汐(晚 8 点效率翻倍)、异兽入侵事件(`!击杀`)
- [x] 宗门战:以 QQ 群为单位每周比拼总修为(`!宗门排行`)
- [x] 社交惩罚:连续不发言效率递减(24h×0.9/48h×0.8/72h+×0.5),发消息后恢复
- [x] 道侣/情缘系统(`!道侣 @某人` / `!道侣 解除`):永久效率+10%
